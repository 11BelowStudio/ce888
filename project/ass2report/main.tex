...
impact on the outcome appear to be `t`, `x5`, and `x14`, in that order. Looking at Figure \ref{fig:ihdpSimpleB}, `t=0` has a somewhat negative
...
the positives. `x14` appears to have no/negligibly negative impact on `y` when `x14=0`\footnote{x14 is a binary value (only 0 or 1), so blue=0, red=1}, with potential for a slightly less negligible positive
impact when `x14=1`.
...
\subsubsection{IHDP simple learners with IPSW `Y|XT*IPSW(X)`}
